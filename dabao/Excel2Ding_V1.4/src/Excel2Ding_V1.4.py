...
class ColumnMapper:
    ...
    DEFAULT_MAPPING = {
        '发起人姓名': ['发起人姓名', '姓名', '提交人', '申请人'],
        '发起时间': ['发起时间', '创建时间', '提交时间', '申请时间'],
        '项目名称': ['项目名称', '项目', '项目简称', '客户项目'],
        '产品线': ['产品线', '产品', '产品类型', '所属产品'],
        '申请状态': ['申请状态', '当前进度', '状态', '审批状态'],
        '特制化比例(%)': ['特制化比例(%)', '特制化比例', '定制比例', '特制化程度'],
        '可常规化比例(%)': ['可常规化比例(%)', '可常规化比例', '标准化比例', '常规化程度'],
        '建议报价(元)': ['建议报价(元)', '报价金额', '报价', '预估金额'],
        '定制内容': ['定制内容', '定制需求', '需求描述', '定制说明'],
        '产品名称': ['软件版本', '产品名称', '软件名称', '产品型号'],
        '原产品主型号': ['硬件情况（分辨率）/原产品主型号', '原产品型号', '硬件型号', '基础型号'],
        '销售部门': ['销售部门', '部门', '所属部门', '销售团队'],
        '销售经理': ['定制人', '销售经理', '销售负责人', '业务负责人']
    }
    ...
    def __init__(self):
        ...
        self.config_path = os.path.join(self.config_dir, 'column_mapping.json')
        # 确保配置目录存在
...
def process_excel(
    ...
    try:
        ...
        for target, source in matched.items():
            if target != '发起时间':  # 跳过发起时间列，因为需要特殊处理
                output_df[target] = filtered[source]
        
        # 2. 添加格式化的时间
        output_df['发起时间'] = filtered['datetime_obj']
        # .dt.strftime('%Y/%m/%d %H:%M')
        
        # 3. 在时间后面添加当前周
        output_df.insert(output_df.columns.get_loc('发起时间') + 1, '当前周', filtered['当前周'])
        
        # 4. 如果设置了产品线替换规则
        if target_product and new_contact:
            # 替换指定产品线对应的对接人
            output_df.loc[output_df['产品线'] == target_product, '发起人姓名'] = new_contact
        
        # 5. 按照 OUTPUT_COLUMNS 重命名列
        output_df = output_df.rename(columns=column_mapper.output_columns)
        
        # 6. 确保输出列的顺序与 OUTPUT_COLUMNS 一致
...
def setup_window(window, title, size, resizable=(1, 1)):
    """统一设置窗口属性"""
    window.title(title)
...
def create_gui():
    ...
    root.resizable(1, 1)  # 禁止调整大小以保持布局一致性
